...
export const usePushNotifications = ({
  ...
  const registerServiceWorker = useCallback(async (): Promise<ServiceWorkerRegistration | null> => {
    ...
    try {
      ...
      logger.info('✅ Service Worker registrado:', { scope: registration.scope, active: !!registration.active });
      return registration;
    } catch (error) {
  ...
  const subscribe = useCallback(async (): Promise<PushSubscription | null> => {
    ...
    try {
      ...
      logger.info('✅ Suscripción push creada:', { endpoint: pushSubscription.endpoint });
