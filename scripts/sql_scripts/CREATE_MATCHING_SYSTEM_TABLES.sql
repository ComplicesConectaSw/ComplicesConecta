-- =====================================================
-- MATCHING SYSTEM - SUPABASE TABLES
-- Fecha: 14/09/2025 08:58hrs
-- Versión: v2.8.1 - Sistema de Matching Completo
-- =====================================================
...
CREATE POLICY "System can create matches" ON public.matches
    FOR INSERT WITH CHECK (true); -- Matches creados por funciones del sistema
...
BEGIN
    ...
    SELECT 
        ...
        p.interests,
        p.avatar_url,
        0::NUMERIC as distance_km, -- TODO: Implementar cálculo real de distancia
        p.is_online,
        p.last_active
...
-- Constraint para email único en auth.users (ya existe por defecto en Supabase)
-- Validación adicional en profiles
ALTER TABLE public.profiles ADD CONSTRAINT unique_email_profiles UNIQUE (email);

-- Constraint para evitar auto-likes
ALTER TABLE public.user_likes ADD CONSTRAINT no_self_likes CHECK (liker_id != liked_id);

-- Constraint para orden consistente en matches
...
COMMENT ON FUNCTION create_mutual_match IS 'Crea match automáticamente cuando hay like recíproco';
COMMENT ON FUNCTION get_user_matches IS 'Obtiene todos los matches de un usuario con información completa';
COMMENT ON FUNCTION get_potential_matches IS 'Obtiene perfiles potenciales para matching con filtros';
COMMENT ON FUNCTION update_user_activity IS 'Actualiza estado online y última actividad del usuario';

-- =====================================================
-- DATOS DE PRUEBA (OPCIONAL - SOLO PARA DESARROLLO)
-- =====================================================

-- Insertar intereses de ejemplo solo si estamos en desarrollo
DO $$
BEGIN
    IF current_setting('app.environment', true) = 'development' THEN
        -- Actualizar perfiles existentes con intereses de ejemplo
        UPDATE public.profiles 
