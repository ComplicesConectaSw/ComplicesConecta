...
-- 18. Trigger para nuevos usuarios
DROP TRIGGER IF EXISTS on_auth_user_created ON auth.users;
CREATE TRIGGER on_auth_user_created
  AFTER INSERT ON auth.users
...
BEGIN
    ...
    AND table_name IN ('profiles', 'user_roles', 'invitations', 'gallery_permissions');
    
    -- Contar funciones críticas
    SELECT COUNT(*) INTO function_count
    FROM information_schema.routines 
    WHERE routine_schema = 'public' 
    AND routine_name IN ('has_role', 'get_user_role', 'handle_new_user');
    ...
    RAISE NOTICE '✅ CORRECCIÓN DE MIGRACIÓN COMPLETADA';
    RAISE NOTICE '📊 Tablas críticas: %', table_count;
    RAISE NOTICE '🔧 Funciones críticas: %', function_count;
    RAISE NOTICE '🔐 Políticas RLS: %', policy_count;
    RAISE NOTICE '🚀 Base de datos corregida y lista para uso';
END $$;
