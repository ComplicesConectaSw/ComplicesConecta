...
BEGIN
    ...
    IF ABS(NEW.amount) > 1000 THEN
        -- Log de auditor├¡a simple (sin tabla audit_logs por ahora)
...
BEGIN
    ...
    IF (user_tokens.monthly_earned + world_id_amount) > user_tokens.monthly_limit THEN
        RETURN jsonb_build_object(
            'success', false,
            'message', 'L├¡mite mensual alcanzado (' || user_tokens.monthly_limit || ' CMPX)'
        );
    END IF;
    ...
    INSERT INTO public.transactions (
        ...
        'Recompensa por verificaci├│n World ID'
    );
...
BEGIN
    ...
    -- Verificar si ya termin├│ el per├¡odo
    ...
    SELECT 
        ...
        'Recuperaci├│n de tokens en staking'
...
BEGIN
    LOOP
        -- Generar c├│digo basado en UUID + contador
...
COMMENT ON FUNCTION "public"."get_post_comments"("post_uuid" "uuid", "page_limit" integer, "page_offset" integer) IS 'Funci├│n para obtener comentarios de un post espec├¡fico';
...
BEGIN
    ...
    IF NOT FOUND THEN
        RETURN jsonb_build_object(
            'success', false,
            'message', 'C├│digo de referido inv├ílido'
        );
    END IF;
    ...
    -- Verificar l├¡mite mensual del invitador
    IF (inviter_record.monthly_earned + referral_amount) > inviter_record.monthly_limit THEN
        RETURN jsonb_build_object(
            'success', false,
            'message', 'L├¡mite mensual alcanzado (' || inviter_record.monthly_limit || ' CMPX)'
        );
    END IF;
...
BEGIN
    ...
    INSERT INTO public.transactions (
        ...
        'Tokens puestos en staking por ' || duration_days || ' d├¡as'
    );
    
    RETURN jsonb_build_object(
        'success', true,
        'message', 'Staking iniciado: ' || amount_param || ' CMPX por ' || duration_days || ' d├¡as',
        'amount', amount_param,
...
CREATE OR REPLACE FUNCTION "public"."validate_token_modification"() RETURNS "trigger"
    ...
    AS $$
BEGIN
    -- Solo permitir modificaciones desde funciones espec├¡ficas o admins
    IF NOT (
        current_setting('application_name', true) LIKE '%supabase%'
    ) THEN
        RAISE EXCEPTION 'Modificaci├│n de tokens no autorizada';
    END IF;
...
COMMENT ON TABLE "public"."user_tokens" IS 'Balances de tokens CMPX/GTK por usuario con l├¡mites mensuales';
...
COMMENT ON COLUMN "public"."user_tokens"."monthly_earned" IS 'Tokens ganados este mes (m├íx 500)';



COMMENT ON COLUMN "public"."user_tokens"."monthly_limit" IS 'L├¡mite mensual de tokens por referidos';
...
CREATE POLICY "Usuarios pueden ver perfiles p├║blicos" ON "public"."profiles" FOR SELECT USING ((NOT "is_blocked"));
